...
static bool trace_uprobe_match_head(struct trace_uprobe *tu,
				    int argc, const char **argv)
{
	...
	return trace_probe_match_args(&tu->tp, argc, argv);
}
			...
			int argc, const char **argv, struct dyn_event *ev)
{
	...
	return strcmp(trace_probe_name(&tu->tp), event) == 0 &&
	   ...
	   trace_uprobe_match_head(tu, argc, argv);
}
...
static bool trace_uprobe_same_type(struct trace_uprobe *a,
				   struct trace_uprobe *b)
{
	if (is_ret_probe(a) != is_ret_probe(b))
		return false;
...
static int register_trace_uprobe(struct trace_uprobe *tu)
{
	...
	if (old_tu) {
		if (trace_uprobe_same_type(tu, old_tu))
