...
static bool trace_kprobe_match_head(struct trace_kprobe *tk,
				    int argc, const char **argv)
{
	...
	return trace_probe_match_args(&tk->tp, argc, argv);
}
			...
			int argc, const char **argv, struct dyn_event *ev)
{
	...
	return strcmp(trace_probe_name(&tk->tp), event) == 0 &&
	    ...
	    trace_kprobe_match_head(tk, argc, argv);
}
				...
				struct trace_event_file *file)
{
	...
	if (file) {
		ret = trace_probe_check_last_file(tp, file);
		if (ret < 0)
			return ret;
		if (ret == 0)	/* Not the last file -- just remove */
			goto out;
...
static bool trace_kprobe_same_type(struct trace_kprobe *a,
				   struct trace_kprobe *b)
{
	if (trace_kprobe_is_return(a) != trace_kprobe_is_return(b))
		return false;

	return trace_probe_same_arg_type(&a->tp, &b->tp);
}
...
static int register_trace_kprobe(struct trace_kprobe *tk)
{
	...
	if (old_tk) {
		if (trace_kprobe_same_type(tk, old_tk))
