...
{
	return tf->symbols ? tf->symbols : "unknown";
}

static unsigned long trace_fprobe_offset(struct trace_fprobe *tf)
{
	return 0;
}
			...
			int argc, const char **argv, struct dyn_event *ev)
{
	...
	return (event[0] == '\0' ||
	    ...
	    trace_fprobe_match_command_head(tk, argc, argv);
}

static unsigned long trace_fprobe_nhit(struct trace_fprobe *tf)
{
	unsigned long nhit = 0;
	int cpu;

	for_each_possible_cpu(cpu)
		nhit += *per_cpu_ptr(fk->nhit, cpu);
...
static struct trace_fprobe *
trace_fprobe_primary_from_call(struct trace_event_call *call)
{
	struct trace_probe *tp;
	...
	return container_of(tp, struct trace_fprobe, tp);
}

static int register_fprobe_event(struct trace_fprobe *tf);
static int unregister_fprobe_event(struct trace_fprobe *tf);

static int fentry_dispatcher(struct fprobe *fp, unsigned long entry_ip, struct pt_regs *regs, void *entry_data);
static void fexit_dispatcher(struct fprobe *fp, unsigned long entry_ip, struct pt_regs *regs, void *entry_data);
					     ...
					     int nargs, bool is_return)
{
	...
	tf->nhit = alloc_percpu(unsigned long);
	if (!tf->nhit)
		goto error;

	tf->symbols = kstrdup(symbols, GFP_KERNEL);
	if (!tf->symbols)
		goto error;
	...
	tf->fp.nr_rethook_nodes = maxactive;
...
static int __register_trace_fprobe(struct trace_fprobe *tf)
{
	...
	return register_fprobe(&tf->fp, tf->symbols, NULL);
}
...
static int append_trace_fprobe(struct trace_fprobe *tf, struct trace_fprobe *to)
{
	...
	if (trace_fprobe_is_return(tf) != trace_fprobe_is_return(old_tf)) {
	...
	if (ret)
		...
		dyn_event_add(&tf->devent, trace_probe_event_call(&tk->tp));
...
static int __trace_fprobe_create(int argc, const char *argv[])
{
	 ...
	 *  - Add fexit probe:
	 *      r[MAXACTIVE][:[GRP/][EVENT]] [MOD:]KSYM [FETCHARGS]
	 *    Or
	 *      f[:[GRP/][EVENT]] [MOD:]KSYM%return [FETCHARGS]
	 *
	 * Fetch args:
	...
	if (isdigit(argv[0][1])) {
		 ...
		 */
		if (maxactive > FPROBE_MAXACTIVE_MAX) {
	...
	ret = traceprobe_set_print_fmt(&tf->tp, ptype);
	if (ret < 0)
		goto error;
...
static int trace_fprobe_release(struct dyn_event *ev)
{
	struct trace_fprobe *tf = to_trace_kprobe(ev);
...
static int trace_fprobe_show(struct seq_file *m, struct dyn_event *ev)
{
	...
	seq_putc(m, trace_fprobe_is_return(tf) ? 'r' : 'f');
		 ...
		 unsigned long ret_ip, struct pt_regs *regs)
{
	...
	trace_probe_for_each_link_rcu(link, &tf->tp)
		__kretprobe_trace_func(tf, entry_ip, ret_ip, regs, link->file);
}
...
static inline void init_trace_event_call(struct trace_fentry *k)
{
	struct trace_event_call *call = trace_probe_event_call(&tk->tp);

	if (trace_fprobe_is_return(tk)) {
