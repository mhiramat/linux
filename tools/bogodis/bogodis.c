...
static void print_code(const insn_byte_t *buf, size_t len)
{
	int i;
	for (i = 0; i < len; i++)
		printf("%02x ", buf[i]);
	printf("%*s", (int)(3 * (8 - len)), "");
}
...
int main(int argc, char *argv[])
{
	...
	while ((ret = read_instruction(stdin, insn_buf, MAX_INSN_SIZE)) > 0) {
		...
		ret = disassemble(&insn, buf, sizeof(buf));
		if (ret < 0) {
			printf("Error: reason %d\n", ret);
			if (verbose)
				dump_insn(stdout, &insn);
		} else {
			print_code(insn.kaddr, insn.length);
			printf("%s\n", buf);
			if (verbose >= 2) {
				printf("format: %s\n",
