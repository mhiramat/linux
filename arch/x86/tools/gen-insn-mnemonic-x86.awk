...
/^[0-9a-f]+\:/ {
	...
	while (i <= NF) {
		...
		if (match(opcode, "^jmpe"))
			opcode = "(jmpe)"	# JMPE is a special one
		if (match(opcode, "^jmp.*"))
			opcode = "jmp"
		...
		if (match(opcode, "^repne")) sub("ne", "nz", opcode)
		...
		if (match(opcode, "f*x(save|rstor)"))
			opcode = opcode " " opnd "|%q:" opcode "64"
		# int XX should have a 8bit
		if (opcode == "int" && opnd == "Ib")
			opnd = "IB"
