...
static int __disasm_gprea(struct disasm_buffer *dbuf, const char *opnd,
			  struct insn *insn, int idx)
{
...
static int disasm_pointer(struct disasm_buffer *dbuf, const char *opnd,
			  struct insn *insn)
{
	const char *type = "(bad)";
...
static int disasm_sib(struct disasm_buffer *dbuf, const char *opnd,
		      struct insn *insn)
{
	...
	if (mod != 0 || base != 5)	/* With base */
		__disasm_gprea(dbuf, opnd, insn, base + rexb);

	if (index != 4)	{	/* With scale * index */
		disasm_printf(dbuf, ",");
		__disasm_gprea(dbuf, opnd, insn, index + rexx);
			    ...
			    struct insn *insn)
{
	...
	if (disasm_syntax_intel(dbuf)) {
		ret = disasm_pointer(dbuf, opnd, insn);
		if (ret < 0)
			return ret;
	}

	if (insn->sib.nbytes)	/* SIB addressing */
		return disasm_sib(dbuf, opnd, insn);
	...
	if (mod == 0 && rm == 5) {	/* displacement only */
		...
		if (insn_rex_b_bit(insn))
			rm += 8;
		__disasm_gprea(dbuf, opnd, insn, rm);
		return disasm_printf(dbuf, ")");
	}
}
			 ...
			 struct insn *insn)
{
	...
	if (ret < 0)
		return ret;
	return disasm_printf(dbuf, "%s%ci)", pfx, *opnd == 'X' ? 's' : 'd');
}
